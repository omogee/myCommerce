...
conn.query('SELECT *,  CONCAT("₦", FORMAT(sellingprice, 0)) AS mainprice FROM product INNER JOIN product_rating using (productId) WHERE `subcat1` = "'+cat+'" LIMIT ? OFFSET ?',[numPerPage,skip],(err,file)=>{
        ...
        res.json({file,numPages,currentPage})
    })
})
})  
app.post("/:productId/rate",(req,res)=>{
...
app.get('/items/search', (req,res) =>{
    ...
    conn.query("SELECT *,CONCAT('₦', FORMAT(sellingprice, 0)) AS mainprice FROM product INNER JOIN product_rating using (productId) WHERE details LIKE '%"+search+"%' OR brand IN ("+brands+") OR size IN ("+sizes+") OR color IN ("+colors+") ORDER BY FIELD(brand, "+brands+")",(err,files)=>{
        if (err) throw err;
        files.map(file => { 
...
app.post('/search', (req,res)=>{
    ...
    conn.query("SELECT *,CONCAT('₦', FORMAT(sellingprice, 0)) AS mainprice FROM product INNER JOIN product_rating using (productId) WHERE details LIKE '%"+search+"%'",(err,files)=>{
        if (err) throw err;
        files.map(file => { 
            file["authur"] = "Eze Ogbonnaya"
            if(files.productrating){      
...
const sql = "SELECT *,CONCAT('₦', FORMAT(sellingprice, 0)) AS mainprice FROM product INNER JOIN product_rating using (productId) WHERE (brand IN ("+brands+") OR size IN ("+sizes+") OR color IN ("+colors+")) and subcat1='"+category+"' ORDER BY FIELD(brand, "+brands+")";
conn.query(sql, (err, file)=>{
    if (err) throw err;
...
app.get('/:category/color', (req,res)=>{
    const cat = req.params.category;
   conn.query('SELECT DISTINCT color FROM product WHERE `subcat1` = "'+cat+'"',(err,files)=>{
       if (err) throw err;
       res.send(files)
   })
})
...
app.get('/:category/size', (req,res)=>{
    const cat = req.params.category;
    conn.query('SELECT DISTINCT size FROM product WHERE `subcat1` = "'+cat+'" ORDER BY size DESC',(err,files)=>{
        if (err) throw err;
        res.send(files)
    })
 })
 app.get('/:category/brand', (req,res)=>{
     const cat = req.params.category;
    conn.query('SELECT DISTINCT brand FROM product  WHERE `subcat1` = "'+cat+'"',(err,files)=>{
        if (err) throw err;
        res.send(files)
    })
 })
