...
import {fetchsavedItem,unsaveItem,saveItem} from "./store"
import {connect} from "react-redux"
import "./main.css"

class UploadsTwo extends Component {
    state = { 
        products:[],
...
const removeBounce = ()=>{
  this.setState({bounce:""})
   ...
   save =()=>{
    ...
    let data={
      productId :this.state.unsavingDetailId,
      details: this.state.unsavingDetail
     }
this.props.saveItem(data)  
 ...
 <div className="col-5 col-md-6" style={{fontSize:"20px",padding:"10px"}}>                     
                           <small > {this.props.othersellerproducts.length > 0 ? "Uploads"  : null}</small>
                               </div>                           
        <div className="col-4 col-m4-3" style={{padding:"10px",display:`${this.props.othersellerproducts.length > 0 ? "block" :"none"}`}}>                
               <small style={{fontSize:"12px",fontWeight:"bold"}} className="text-muted"> (1-{this.props.othersellerproducts.length}) of {this.props.sellernumOfRows} items</small>
                               </div>                                          
               <div style={{padding:"10px",display:`${this.props.othersellerproducts.length > 0 ? "block" :"none"}`}}>
              ...
              <div className="savemodaldiv" ref={(a) => this.savemodaldiv =a} id="savemodaldiv" style={{display:`${this.state.displayaskdiv}`,zIndex:"3",width:"100%",height:"100%",backgroundColor:"rgba(0,0,0,0.4)"}}>
              <div className="savediv"  style={{backgroundColor:"white"}}>
              <center>
               <p style={{padding:"50px"}}>
             <b>{this.props.userdetails.savedItems && JSON.parse(this.props.userdetails.savedItems).includes(parseInt(this.state.unsavingDetailId)) ? "`UnSave`" : "`Save`"}</b>  {this.state.unsavingDetail}
               </p>
               ...
               <div className="col-6">
              <button className="btn btn-success" onClick={()=>this.save(this.state.unsavingDetailId)} style={{padding:"8px",color:"white",width:"100%",boxShadow:"2px 3px lightgrey"}} >Proceed</button>
              </div>         
               </div>
     </center>
     </div>
     ...
     <div className="row" style={{backgroundColor:`${this.props.userdetails.background==="black"?"rgb(38,38,38)":"rgb(242,242,242)"}`}}>
       {this.props.othersellerproducts.length > 0 ? this.props.othersellerproducts.map((product) =>          
 <div className="col-6 col-md-4 col-lg-2 rowclass"  style={{display:`${this.state.view==="grid" ? "block" : "none"}`,padding:"3px"}}  key={product.productId} >        
      <div className="col-12 d-md-none">
          <span className={`${this.state.bounce} ${this.props.userdetails.savedItems && JSON.parse(this.props.userdetails.savedItems).includes(parseInt(product.productId))?"fa fa-heart":"far fa-heart"}`} style={{fontSize:"20px",position:"absolute",top:"10px",color:"orange"}}></span>
        </div>
...
<small style={{float:"left",width:"100%",textTransform:"capitalize"}}>{product.brand}</small>
     ...
     <small onClick={()=>this.openDetails(product.productId)} style={{color:`${this.props.userdetails.background === "black" ? "white" : "black"}`,fontSize:"11px"}}>
       {product.details.length > 50 ? product.details.slice(0,50)+ "..." : product.details +"-"+ product.model +"-"+ product.color}</small>  
        </div> 
       ...
       <div className="outer">     
        ...
        <center className="mt-2"  style={{display:`${window.innerWidth >= 600 ? this.state.viewaddtocartbutton : `none`}`,width:`${this.state.viewcartbtnwidth}`}}>
        {this.props.userdetails.userId === this.props.otheruserdetails.userId ? 
   ...
   :  
   <div style={{border:"1px soliud lightgrey"}}>    
   <button  type="button" style={{padding:"0px",width:"65%"}} ref={this.detailsRef} className="btn addtocartbtn " onClick={()=>this.addtocart(product.productId)}>
   <small style={{fontSize:"12px"}}>Add to cart</small>
   </button>
   <i onClick={()=>this.askToUnsave({details:product.details,id:product.productId})} className={this.props.userdetails.savedItems && JSON.parse(this.props.userdetails.savedItems).includes(parseInt(product.productId)) ?`fa fa-heart ${this.state.bounce}`: `far fa-heart ${this.state.bounce}`} style={{padding:"0px 6px",color:"orange",width:"30"}}></i>
   <br/>
      ...
      {this.props.othersellerproducts.length === 0 && !this.props.loading  ?
...
const mapStateToProps =(store)=>{
  return{           
    othersellerproducts:store.othersellerproducts,
...
export default connect(mapStateToProps,mapDispatchToProps)(UploadsTwo);